...
var graph = (function(graph) {
  ...
  var min_zoom = 0.15;
  var max_zoom = 1.8; //7
  var svg = d3.select(".body-content").append("svg").attr({
    width: w,
    height: h,
    viewBow: [0, 0, w, h].join(' ')
  });
  ...
  function formatGraph(){
      ...
      for(var open in openNode){
          ...
          if(!(node.name in nodesDict)){
              nodesDisplayed[nodesDisplayed.length] = node;
              nodesDict[node.name] = node.name;
          }

          for(var j = 0; j < links.length; j++){

            if(links[j].source == node){
              //checking if the node wasn't listed before
              if(!(links[j].target.name in nodesDict)){
                //checking to see if the person is an entity different from a person
                nodesDisplayed.push(links[j].target);
                nodesDict[links[j].target.name] = links[j].target.name;
              }
            }
          }
      }
      ...
      for(var open in openNode){
          ...
          for(var j = 0; j < links.length; j++){
            ...
            // <name>-<name>
              ...
              if( !(linkAux in linksDict) && links[j].source == node){
                  linksDisplayed.push({
                      "source": pstLink,
                      "target": checkLinkPosition(links[j].target.name, nodesDisplayed),
